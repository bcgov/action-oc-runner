...
CLUSTER=""
if [[ "${OC_SERVER}" =~ ^https://api\.([a-z0-9-]+)\.devops\.gov\.bc\.ca:6443$ ]]; then
  CLUSTER="${BASH_REMATCH[1]}"
fi

if [[ "${OC_RELAY}" == *"{cluster}"* ]] && [ -z "${CLUSTER}" ]; then
  echo "::error::oc_relay contains {cluster} but oc_server '${OC_SERVER}' is not https://api.<cluster>.devops.gov.bc.ca:6443. Set oc_relay to a full host or pass oc_relay: '' to connect directly." >&2
  exit 1
fi
